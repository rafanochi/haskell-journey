...
oddFactorial 0 = 1
oddFactorial x = returnOdd x * oddFactorial (x-1)
    where returnOdd x = if odd x then x else 1
...
leftpad str x = spaces (x - length str) ++ str 
    where
        spaces y
            | y <=0 = ""
            | otherwise = " " ++ spaces (y - 1)
...
countdown x = "Ready! " ++ strnumber x 
        where
            strnumber 0 = "Liftoff!" 
            strnumber x = show x ++ "... " ++ strnumber (x - 1)
...
smallestDivisor x = head $ filter (\y -> x `mod` y == 0) [2..x]
...
biggestPrimeAtMost x = last $ filter isPrime [2..x]
